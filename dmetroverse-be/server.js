...
app.get("/", (req, res) => {
  ...
  res.send(
    "This server uses cors to access delhimetrorail. If you need access, contact the developer (dmetroverse@gmail.com)."
  );
});
...
app.get("/:origin/:destination/:type?", async (req, res) => {
  ...
  let type = req.params.type || "least-distance";

  if (type !== "mi") {
    type = "least-distance";
  } else {
    type = "minimum-interchange";
  }

  let currentTime = getCurrentTimeInIST();
  let apiEndpoint = `https://backend.delhimetrorail.com/api/v2/en/station_route/${origin}/${destination}/${type}/${currentTime}`;
